...
window.Vaadin.Flow['_vaadintheme_mybank_globalCss'] = window.Vaadin.Flow['_vaadintheme_mybank_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_mybank_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_mybank_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_mybank_globalCss'].push(target);
  }
  if (!document['_vaadintheme_mybank_componentCss']) {
    
    document['_vaadintheme_mybank_componentCss'] = true;
  }
